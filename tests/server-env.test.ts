...
describe("readServerEnv", () => {
  ...
  it("treats an unset variable as absent", () => {
    ...
    expect(readServerEnv({ requiresAccessKey: false }).API_ACCESS_KEY).toBeUndefined();
  });
  ...
  // An endpoint compares against a bearer token, which cannot carry leading or
...
// A Route Handler has no authentication of its own and no middleware in front
...
//
// What decides it is the wiring, which declares it through
// `requiresAccessKey` -- never which variables happen to be exported on the
// machine.
describe("readServerEnv with a billed endpoint wired", () => {
  ...
  it("names API_ACCESS_KEY as the variable at fault, and no credential value", () => {
    vi.stubEnv("SOME_PROVIDER_CREDENTIAL", "an-example-value");
    vi.stubEnv("API_ACCESS_KEY", undefined);
...
describe("readServerEnv with nothing billed wired", () => {
  ...
  // that exports a provider credential for something else would refuse to
  // start, build, or even load this suite, while nothing billed was wired.
