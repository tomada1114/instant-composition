...
| [0006](adr/0006-persistence-on-dynamodb.md)                     | DynamoDB with one partition per learner; async, learner-bound stores that commit per command, with projections instead of full-log replay.           | Accepted; `dev` table protection superseded by 0014                |
...
| [0009](adr/0009-aws-topology-environments-and-operations.md)    | CloudFront → API Gateway HTTP API → Lambda, no VPC; infrastructure as code; "portfolio grade" operations.                                            | Accepted; `dev` table protection superseded by 0014                |
...
| [0014](adr/0014-defer-dev-table-protection-on-the-free-plan.md) | Create the `dev` table without point-in-time recovery or deletion protection, retained by CloudFormation, and turn both on at the Paid-plan upgrade. | Accepted                                                           |
