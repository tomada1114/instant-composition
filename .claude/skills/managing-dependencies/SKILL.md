...
pnpm web:build          # vite build — the web client's bundle, which no unit test builds
pnpm db:up && pnpm test:smoke && pnpm test:dynamodb   # the built stack, and DynamoDB local
pnpm test:coverage      # the coverage floors, which a swapped dependency can move
```

Everything after the install is what `pnpm check:source` runs, so one green run of that
(with DynamoDB local up) covers them all. One narrower run is worth naming, because the
gate reports its failure only as a wall of output: the suite for the module that
actually consumes the bumped package, run on its own.

```bash
pnpm exec vitest run tests/<module>.test.ts
```

A `zod` bump surfaces first in whichever contract suite parses with it; a `react`,
TanStack or `use-intl` bump in the `tests/web-*.test.tsx` suites; an AWS SDK bump in
`pnpm test:dynamodb`. No suite reaches a live service either way.
...
- Add or bump through `pnpm add`, never by hand-writing a version into `package.json`. A
  ...
  something older.
- This is a pnpm workspace, so say whose dependency it is. `pnpm add` at the root
  refuses with `ERR_PNPM_ADDING_TO_ROOT` until `-w` makes the root explicit; a workspace
  package's or app's own dependency is
  `pnpm --filter @instant-composition/<dir> add <pkg>`. A package under `packages/` or
  an app under `apps/` may import only what its row in `eslint.config.mjs`'s boundary
  tables names, and `tests/boundaries.test.ts` holds each manifest to that row, so a new
  dependency there is also an edit to those tables — a boundary decision, argued in the
  PR.
...
- `strictDepBuilds` plus `allowBuilds`: an install-time lifecycle script from a
  ...
  outcome, not a bug to route around. Each package that wants to run one is a
  case-by-case review, not a standing exception. Three questions settle an entry: what
  the script actually does (download a binary, compile native code, probe for a prebuilt
  one); whether anything this repository runs needs its result, or it belongs to a
  feature never turned on; and whether the package already ships a prebuilt platform
  binary as an optional dependency, making the script a fallback rather than the only
  path. `false` is as much a decision as `true` — it records that the script was
  reviewed and refused, so the next install failure is not answered with a reflexive
  `true`. Read the file's comments for the ruling each entry carries; adding a `true`
  one carries the same review weight as adding a new dependency.
...
## An MCP server is a dependency outside pnpm's graph

`.mcp.json` declares no server today. A server added there runs through `pnpm dlx`,
straight from the npm registry, with no `package.json` entry and no `pnpm-lock.yaml`
line. Every protection above is a `pnpm`-install-time mechanism, so none of it reaches
this file: no lockfile pins the resolved version, `minimumReleaseAge` never sees a `dlx`
fetch, and `pnpm audit` never walks a graph this file is not part of. Left unpinned
(`<package>@latest`), the same commit runs a different tool depending on when it happens
to be fetched — no lockfile, no cooldown, no review record, for a server that runs
inside real development sessions.
...
- Pin an exact version in the `args` array (`<package>@<version>`) — never `@latest` and
  never a `^`/`~` range. A range here has no lockfile to freeze its resolution, so it
  would still float to whatever is newest each time `pnpm dlx` runs; only an exact
  string is reproducible outside pnpm's graph.
- Resolve the version by hand with `npm view <package> version`, and check how recently
  it was published with `npm view <package> time --json` before adopting it — the
  cooldown above exists for exactly this reason (a freshly published version installed
  unreviewed), and nothing enforces it here, so apply it by eye: prefer a version that
  has been out for at least the `minimumReleaseAge` window over the newest one.
