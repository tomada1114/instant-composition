...
- This is a pnpm workspace, so say whose dependency it is. `pnpm add` at the root
  ...
  package's own dependency is `pnpm --filter @instant-composition/<dir> add <pkg>`. A
  package under `packages/` may declare only what AGENTS.md's "The workspace" allows it
  to import, and `tests/boundaries.test.ts` fails on anything else in its manifest, so a
  new dependency there is also an edit to that table.
