...
description: >
  ...
  the local stand-in authenticator, apps/api's environment, and running it with pnpm api
  against DynamoDB local. Use when an endpoint is added to the contract, a log field is
  added or read, a request is refused with an unexpected code, or the local API will not
  start.
---
...
`ERR_*` vocabulary (`designing-errors`); where a test goes (`placing-tests`).
...
`.env` file is loaded, so the names come from the shell. `pnpm api` runs
...
The learner table is created on first start. The catalog is read from
`dist/catalog/en/ja.json`; the start-up line says whether it was readable, and the next
request that needs it reads it again.
...
exercises, which is why it only wires what is tested elsewhere.
