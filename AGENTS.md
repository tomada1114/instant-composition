...
- Never read or write `.env*` (the `.example`, `.sample` and `.template` variants are
  fine) or anything under `secrets/`. A `.env` in a checkout may hold a real credential,
  so reading one is already a disclosure whether or not anything is written back: no
  `cat`, no `grep`, no copy to a temp path, and never a value out of it onto a command
  line. `apps/api/src/env.ts` is the list of names a local run reads, and `.env.example`
  ships every one of them with an empty value — those two are what to open when you need
  to know what exists.
- Read or edit `.claude/settings.local.json` only when the owner asks for it in that
  session: it holds the personal permission rules, so changing it changes what an agent
  may run.
